...
#ifdef _WIN32
    #ifdef BUILDING_DLL
        #define LLIST_API __declspec(dllexport)
    #else
        #define LLIST_API __declspec(dllimport)
    #endif
#else
    #define LLIST_API
#endif
 ...
 *         and the global variable `llist_errno` is set with the appropriate value
 */
 ...
 *         appropriate value
 */
LLIST_API llist_node_t llist_find_node(llist_t *list, node_ident_t identifier,
                                     void *arg);
